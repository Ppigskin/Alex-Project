...
Computer4::Computer4(int i, Grid *game):ownership { i }, g{game} {}
...
sort(other_attack.begin(), other_attack.end, compare);
sort(my_attack.begin(), my_attack.end(), compare);
...
bool Computer4::compare (Info a, Info b){
  int aw = 0;
  int bw = 0;
  Piece * ap = g->get_piece(a.col, a.row);
  Piece * bp = g->get_piece(b.col, b.row);
  if(ap == nullptr) {aw = 0;}
  else  {aw = ap->weight;}
  if(bp == nullptr) {bw = 0;}
  else  {bw = bp->weight;}
